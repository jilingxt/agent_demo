...
3. 何时：提取明确时间，例如“某年某月某日某时许”。
4. 何地：提取明确案发或行为发生地点，例如“某市某区某场所”；不要把询问、制作笔录或登记地点误作案发地点。
...
- 财物损坏、拿取、转移等行为要提炼为短句，例如“甲某损坏涉案物品”“乙某转移相关财物”，不要复制整段事情经过。
- 示例中的角色、时间、地点和物品都是占位符，不得复制到实际输出；输出实体必须来自当前材料。
...
- legal_query_terms 可选，用简短、可检索的法律事实概念概括当前断言，例如行为方式、对象、结果或身份条件；不得填写条文编号、罪名或“已构成”等结论。没有可靠概念时返回空数组。
- element_role 可选，仅在材料明确涉及证明链位置时填写：actor_attribution（行为人归属）、legal_context（法定情境）、legal_element（其他构成事实）；不能确定时留空。
- evidence_span 必须保留支撑该断言的最短原文片段，便于审计；无法可靠确定 predicate 或 stance 时不要猜测，应返回空 facts。
  ...
  "facts": [
    {
      ...
      "evidence_span": "...",
      "legal_query_terms": ["行为方式", "对象或结果"],
      "element_role": "actor_attribution|legal_context|legal_element|",
      "confidence": 0.86,
      "source_material_id": "..."
    }
  ]
