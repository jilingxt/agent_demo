...
SHEET_ROWS = {
    ...
    "参数需求清单": [
        [
            "参数类别", "模型ID", "节点/谓词", "参数名称", "当前来源", "是否需真实数据",
            "所需统计量", "估计方法", "最小分组维度", "法律边界", "备注",
        ],
        ["根节点先验", "全部案件族", "各根事实节点", "prior", "专家先验", "建议", "核验为真/假/未知的数量", "Beta后验或分层Beta", "案件族×谓词×时间窗", "不得解释为有罪概率", "按月离线更新"],
        ["条件分布", "全部案件族", "各派生事实节点", "intercept/weights", "专家先验", "需要", "每种父节点状态下子节点真/假/未知数", "加权二项逻辑回归或Beta-CPT", "模型版本×父节点组合", "只推断事实要素", "保留未校准标记"],
        ["来源观测", "证据融合", "角色×谓词×立场", "sensitivity/FPR", "待统计", "需要", "TP/FP/FN/TN及未知数", "Beta-Binomial并进行分层收缩", "来源类别×谓词×立场", "不得按角色直接设固定可信度", "小样本不得单独发布"],
        ["抽取质量", "抽取器", "字段或谓词", "precision/recall/F1", "待统计", "需要", "TP/FP/FN/TN及未知数", "混淆矩阵", "抽取器版本×字段", "只评价抽取准确性", "不代表事实真实性"],
        ["来源依赖", "证据融合", "来源组", "dependency", "人工规则", "需要", "共同来源、复制、转述和共同制作计数", "分组去重或依赖模型", "来源A×来源B×依赖类型", "同源材料不得重复加权", "先人工复核"],
        ["权威锚定", "权威校验", "专业结论范围", "validity/scope", "法律和人工复核", "不用于真值率学习", "有效性、适用范围、替代或失效状态", "确定性校验", "文书类型×版本", "只锚定专业范围内事实", "普通否认不直接削弱"],
        ["法律规则", "规则引擎", "年龄/数额/次数/抗辩/边界", "threshold/rule", "现行法律", "禁止", "不采集训练统计", "确定性规则和人工法律审查", "法域×生效时间", "不得进入贝叶斯训练", "版本化保存"],
    ],
    "原子事实核验": [[
...
ENUMS = {
    ...
    "YesNoDataValues": ("真实数据需求", ["需要", "建议", "禁止", "不用于真值率学习"]),
}

VALIDATIONS = {
    "参数需求清单": {"F": "YesNoDataValues"},
...
FORMULAS = {
    ...
    "来源观测统计": {
        "M2": '=IF(OR(E2="",K2="",L2="",K2<=0,L2<=0),"",IF(E2="真阳性率",IF(F2="","",K2+F2),IF(E2="假阳性率",IF(G2="","",K2+G2),"")))',
        "N2": '=IF(OR(E2="",K2="",L2="",K2<=0,L2<=0),"",IF(E2="真阳性率",IF(H2="","",L2+H2),IF(E2="假阳性率",IF(I2="","",L2+I2),"")))',
