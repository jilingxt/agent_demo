...
最新版本已从简单的事实列表升级为兼容旧接口的轻量 EvidenceGraph。
...
- `EvidenceGraph`：兼容 `CaseGraph`，同时包含 `facts`、`nodes`、`edges`。
...
读取静态 JSONL 法律库，根据案件类型、行为事实和关键词检索可能相关的法律依据。
...
4. 将 `LegalRetrievalTool` 升级为 `LegalKnowledgeBaseTool`，支持文档入库、切片、更新、软删除和混合检索；
5. 将 Judge Agent、Review Agent 与最终审查能力合并为更完整的 FinalConflictAgent；
6. 在报告中明确区分高支持事实、争议事实、低置信事实和需人工核实事项。

## 12. 总结

最新版本已经完成从“事实列表 demo”到“轻量证据图 demo”的第一步升级。系统仍保持简单、可运行、可测试，同时为后续法律知识库、复杂关系推断、最终审查 Agent 和业务化界面预留了清晰演进路径。
